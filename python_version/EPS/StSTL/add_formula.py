# Auto-translated from ./EPS/StSTL/add_formula.m

def add_formula(formula,varargin):
...
if ~isscalar(ischar(formula))
    fprintf('You should add one formula expression at a time. Quit.\n')
    add_result = 0
elseif ~ischar(formula)
    fprintf('This formula is not a string! Quit.\n')
    add_result = 0
elseif ~strcmp(')',formula(end))
    fprintf('Formulas should end by )! Quit.\n')
    add_result = 0
# end
...
switch var_num
    ...
    case 0
        ...
        neg_prefix = 0
    otherwise
        fprintf('too many input arguments! Quit.\n')
        add_result = 0
# end
if ~isscalar(formu_index)  or  any(formu_index < 0)
    fprintf('Invalid formu_index. Quit.\n')
    add_result = 0
# end
if size(sat_time_hint,1) != 1
    fprintf('sat_time_hint should be a row! Quit.\n')
    add_result = 0
# end
if 0 == add_result
    return
...
if formu_index > 0
    if StSTL.display == 1
        fprintf(['In add_formula(): ',StSTL.formu_str{formu_index},...
...
# end
    if ~strcmp(formula,StSTL.formu_str{formu_index})...
             or  any(StSTL.formu_time{formu_index} != sat_time_hint)...
             or  any(StSTL.formu_neg{formu_index} != neg_prefix)
...
# end
     ...
     if StSTL.display == 1
         fprintf(['In add_formula(): ',formula,' given by user',' with sat_time_hint = ',...
             num2str(sat_time_hint),'\n'])
# end
    [is_found,formu_index] = search_track_formula(formula,sat_time_hint,neg_prefix)
    if is_found
        fprintf(['formula ',formula,' with sat_time_hint ',num2str(sat_time_hint),...
...
switch formula(1:(left_par(1) - 1))
    case 'Not' # negation, syntex: 'Not(f)'
        neg_prefix = neg_prefix + 1
        add_result = add_neg(formula((left_par(1) + 1):(end - 1)),formu_index,sat_time_hint,neg_prefix)
    case 'Until' # until, syntex: 'Until(f1,f2,t_start,t_end)'
        add_result = add_until(formula((left_par(1) + 1):(end - 1)),formu_index,sat_time_hint,neg_prefix)
    case 'Weak' # until, syntex: 'Weak(f1,f2,t_start,t_end)'
        add_result = add_weak(formula((left_par(1) + 1):(end - 1)),formu_index,sat_time_hint,neg_prefix)
    case 'And' # and, syntex: 'And(f1,...,fm)'
        add_result = add_and(formula((left_par(1) + 1):(end - 1)),formu_index,sat_time_hint,neg_prefix)
    case 'Global' # always, syntex: 'Global(f,t_start,t_end)'
        add_result = add_always(formula((left_par(1) + 1):(end - 1)),formu_index,sat_time_hint,neg_prefix)
    case 'Or' # or, syntex: 'Or(f1,...,fm)'
        add_result = add_or(formula((left_par(1) + 1):(end - 1)),formu_index,sat_time_hint,neg_prefix)
...
# end
    ...
    case 'AP' # atomic proposition, syntex: 'AP(n,t)' or 'AP(n)'
        ...
        add_result = add_AP(formula((left_par(1) + 1):(end - 1)),formu_index,sat_time_hint,neg_prefix)
    otherwise
        fprintf(['Invalid sub formula ''',formula(1:(left_par(1) - 1)),''' detected. Quit.\n'])
        add_result = 0
        return
# end
