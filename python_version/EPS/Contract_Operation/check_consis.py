# Auto-translated from ./EPS/Contract_Operation/check_consis.m

def check_consis(contract, encoding_style):
    # TODO: define outputs: out
#CHECK_COMPAT Check the consistency of a given contract, i.e., the
...
if diagnostic1.problem == 0
    out = 1
    fprintf(['(',contract.A,',',contract.orig_G,') is consistent\n'])
    if disp_feas == 1
...
# end
    ...
    return
elseif diagnostic1.problem == 1  and  strcmp(encoding_style, 'equivalent')
    out = 0
    fprintf(['(',contract.A,',',contract.orig_G,') is inconsistent\n'])
    fprintf('\tcheck_consis() solvertime: #3.3f\n', diagnostic1.solvertime)
    fprintf(['\tencoding style: ', encoding_style, ', ',diagnostic1.info, '\n'])
    contract_checking = 0;              # indicate that contract checcking terminates
    return
elseif strcmp(encoding_style, 'equivalent')
    out = -1
    fprintf(['no conclusion on whether (',contract.A,',',contract.orig_G,') is consistent or not\n'])
    fprintf('\tcheck_consis() solvertime: #3.3f\n', diagnostic1.solvertime)
