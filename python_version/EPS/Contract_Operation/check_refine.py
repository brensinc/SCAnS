# Auto-translated from ./EPS/Contract_Operation/check_refine.m

def check_refine(contract1, contract2, encoding_style):
    # TODO: define outputs: out
...
if diagnostic1.problem != 1  and  diagnostic1.problem != 0     # maybe gurobi is not applicable
    options = sdpsettings('solver','bmibnb','bmibnb.upper','fmincon','verbose',verbose)
    diagnostic1 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
if diagnostic1.problem != 1  and  diagnostic1.problem != 0     # if still does not work, let Yalmip choose a solver
    options = sdpsettings('verbose',verbose)
    diagnostic1 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
if diagnostic1.problem == 1
    any_u_A1_imply_A2 = 1
elseif diagnostic1.problem == 0  and  strcmp(encoding_style, 'equivalent')
    out = 0
...
if diagnostic2.problem != 1  and  diagnostic2.problem != 0     # maybe gurobi is not applicable
    options = sdpsettings('solver','bmibnb','bmibnb.upper','fmincon','verbose',verbose)
    diagnostic2 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
if diagnostic2.problem != 1  and  diagnostic2.problem != 0     # if still does not work, let Yalmip choose a solver
    options = sdpsettings('verbose',verbose)
    diagnostic2 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
if diagnostic2.problem == 1
    any_u_G2_imply_G1 = 1
elseif diagnostic2.problem == 0  and  strcmp(encoding_style, 'equivalent')
    out = 0
    fprintf(['(',contract2.A,',',contract2.orig_G,') does not refine (',contract1.A,',',contract1.orig_G,')\n'])
...
if any_u_A1_imply_A2 == 1  and  any_u_G2_imply_G1 == 1
    out = 1
...
if diagnostic3.problem != 1  and  diagnostic3.problem != 0     # maybe gurobi is not applicable
    options = sdpsettings('solver','bmibnb','bmibnb.upper','fmincon','verbose',verbose)
    diagnostic3 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
if diagnostic3.problem != 1  and  diagnostic3.problem != 0     # if still does not work, let Yalmip choose a solver
    options = sdpsettings('verbose',verbose)
    diagnostic3 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
...
if diagnostic4.problem != 1  and  diagnostic4.problem != 0     # maybe gurobi is not applicable
    options = sdpsettings('solver','bmibnb','bmibnb.upper','fmincon','verbose',verbose)
    diagnostic4 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
if diagnostic4.problem != 1  and  diagnostic4.problem != 0     # if still does not work, let Yalmip choose a solver
    options = sdpsettings('verbose',verbose)
    diagnostic4 = optimize(StSTL.MIP_cons, SMPC.perf_func, options)
# end
