# Auto-translated from ./StContracts/Contract_Operation/check_compat.m

def check_compat(contract, encoding_style):
    # TODO: define outputs: out
#CHECK_COMPAT Check the compatibility of a given contract, i.e., if the
...
if diagnostic1.problem == 0
    out = 1
    fprintf(['contract (',contract.A,',,,',contract.G,') is compatible\n'])
    if disp_feas == 1
...
elseif diagnostic1.problem == 1  and  strcmp(encoding_style, 'equivalent')
    out = 0
    fprintf(['contract (',contract.A,',,,',contract.G,') is incompatible\n'])
    fprintf('\tcheck_compat() solvertime: #3.3f\n', diagnostic1.solvertime)
    fprintf(['\tencoding style: ', encoding_style, ', ',diagnostic1.info, '\n'])
    contract_checking = 0;              # indicate that contract checcking terminates
    return
