# Auto-translated from ./StContracts/StSTL/search_track_formula.m

def search_track_formula(str,sat_time_hint,neg_prefix):
    # TODO: define outputs: is_found,formu_index
...
if StSTL.total_formu > 0  and  StSTL.repeat_check == 1
    ...
    for i = 1:length(formu_index)
        ...
        if size(sat_time_old_formula,2) == size(sat_time_hint,2)
            if all(sat_time_old_formula == sat_time_hint)...
                     and  all(neg_prefix_old_formula == neg_prefix)
                tmp(i) = 1
# end
# end
# end
...
elseif ~any(formu_index) # it is new, we should keep track of the str
    ...
    if 1 == StSTL.display  and  1 == StSTL.repeat_check
        fprintf(StSTL.fid, ['In search_track_formula(): formula ',str,...
            ' with sat_time_hint = ',num2str(sat_time_hint),...
            ', neg_prefix = ',num2str(neg_prefix),...
            ' is new and tracked. formu_index = ',num2str(formu_index),'\n'])
    elseif 1 == StSTL.display  and  0 == StSTL.repeat_check
        fprintf(StSTL.fid, ['In search_track_formula(): formula ',str,...
            ' with sat_time_hint = ',num2str(sat_time_hint),...
            ', neg_prefix = ',num2str(neg_prefix),...
