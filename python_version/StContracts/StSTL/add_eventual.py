# Auto-translated from ./StContracts/StSTL/add_eventual.m

def add_eventual(str,formu_index,sat_time_hint,neg_prefix):
    # TODO: define outputs: add_result
#ADD_EVENTUAL Encode an EVENTUAL formula by assigning binary variables to its 
...
#                    - t, indicating that the sat_time_hint of f is
#                         [t + t_start, t + t_end]
...
if ~isscalar(sat_time_hint)
    error('In add_eventual(), sat_time_hint has to be a scalar.')
# end
if size(sat_time_hint,2) > 1  and  all(sat_time_hint == -1)
    error(['In add_eventual(), if sat_time_hint = [-1,...,-1], then just ',...
           'set sat_time_hint = -1.'])
# end
if neg_prefix > 1
...
if comma_num == 0  or  comma_num == 1
    error('add_eventual() needs its formula string to have three parameters!')
# end
...
if 1 == StSTL.display
    fprintf(StSTL.fid, ['In add_eventual()',...
...
if t_start < 0  or  t_end < 0  or  t_start > t_end
    error('In add_eventual(), invalid t_start or t_end!')
# end
if sat_time_hint != -1  and  sat_time_hint < 0
    error('In add_eventual(), illegal sat_time_hint!')
...
#     else
#         error('In add_eventual(), there should be no more than 1 negation prefix!')
#     end
# end

equiv_str = ['Or(',sub_formula,')']

[is_found,equiv_index] = search_track_formula(equiv_str,t_start:1:t_end,neg_prefix)
if is_found  and  StSTL.display == 1
    fprintf(StSTL.fid, ['In add_eventual(), formula ',StSTL.formu_str{equiv_index},...
